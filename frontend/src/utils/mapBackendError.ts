...
export function mapBackendError(error: any): MappedError {
  ...
  // Check for site deleted errors
  if (
    rawError.includes('site has been deleted') ||
    rawError.includes('no longer available') ||
    rawError.includes('Site has already been deleted')
  ) {
    return {
      userMessage: 'This website has been permanently deleted and is no longer available.',
      rawError,
    };
  }
