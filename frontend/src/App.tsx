...
import { useSiteDeletedState } from './hooks/useQueries';
...
function App() {
  ...
  const { data: isSiteDeleted, isLoading: siteDeletedLoading, isFetched: siteDeletedFetched } = useSiteDeletedState();
  ...
  // Check if site is deleted and show dedicated screen (top priority gate)
  if (isSiteDeleted === true) {
    return <SiteDeletedScreen />;
  }
  ...
  return (
    <AppShell onNavigate={setCurrentPage} siteDeleted={isSiteDeleted || false}>
      {renderPage()}
    </AppShell>
  );
}
